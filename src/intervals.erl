...
-export_type([interval/0,
              ...
              right_bracket/0]).
%% for tester:
-export_type([invalid_interval/0,
              invalid_simple_interval/0,
              simple_interval/0]).
...
-spec new(non_neg_integer()) -> interval().
new(X)
  when is_integer(X) and (X >= 0) ->
    normalize_simple({element, X}).
...
%% If A > B then interval may wraps around, for example [A,B) -> (B,A]
%% The special cases '(A,A)', '[A,A)', '(A,A]' translates to empty interval
%% Only non_negative integers are possible in intervals + infinity atom.
-spec new(left_bracket(), key() | ?PLUS_INFINITY, key() | ?PLUS_INFINITY, right_bracket()) ->
                 interval().
new(LeftBr, Begin, End, RightBr)
  when ((LeftBr  =:= '(') or (LeftBr =:= '[')) and
       ((RightBr =:= ')') or (RightBr =:= ']')) and
       ((Begin =:= infinity) or (is_integer(Begin) and (Begin >= 0))) and
...
normalize_simple({interval, _LeftBr, X, X, _RightBr}) ->
    [];
normalize_simple({interval, _LeftBr, ?MINUS_INFINITY, ?PLUS_INFINITY, _RightBr}) ->
    [all];
normalize_simple({interval, _LeftBr, ?PLUS_INFINITY, ?MINUS_INFINITY, _RightBr}) ->
    [all];
normalize_simple({interval, _LeftBr, X, ?PLUS_INFINITY, _RightBr}) ->
    [{interval, _LeftBr, X, ?PLUS_INFINITY, ')'}];
normalize_simple({interval, _LeftBr, ?PLUS_INFINITY, X, RightBr}) ->
    [{interval, wrap_border(RightBr), X, ?PLUS_INFINITY, ')'}];
normalize_simple({interval, _LeftBr, A, B, _RightBr}) when (A > B) ->
    [{interval, wrap_border(_RightBr), B, A, wrap_border(_LeftBr)}];
normalize_simple({interval, _LeftBr, A, B, _RightBr}) when (A < B) ->
...
-spec intersection_simple(A::simple_interval() | [], B::simple_interval() | []) ->
                                 simple_interval() | [].
...
-spec intersection_simple_element(A::{element, key()}, B::simple_interval()) ->
                                         {element, key()} | [].
...
interval_sort({interval, A0Br, A0, _A1, _A1Br} = A,
              {interval, B0Br, B0, _B1, _B1Br} = B) ->
...
-spec get_elements_list(Interval::interval(), Elements::[key()], RestInt::[simple_interval()]) ->
                               {Elements::[key()], RestInt::[simple_interval()]}.
...
-spec tester_create_continuous_interval(left_bracket(), key(), key(), right_bracket()) ->
                                               continuous_interval().
...
-spec normalize_simple_bounds({interval, left_bracket(), key(), key()
                               | ?PLUS_INFINITY, right_bracket()}
                              | {element, key()} | all)
...
is_well_formed([_|_] = List) ->
    ...
    %% pairwise non-overlapping:
          (lists:flatten([intersection_simple(A, B)
                          || A <- List, B <- List, A =/= B]) =:= []).
